...
public:
	...
	void initResources(VkRenderPass renderPass, VkQueue copyQueue)
	{
		...
		shaderStages[0] = example->loadShader(example->getAssetPath() + "shaders/ui.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = example->loadShader(example->getAssetPath() + "shaders/ui.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
